  ...
  for (i = candidate.begin (); !(i == candidate.end ()); i++)
    {
      ...
      double rosa=tempv.z-nodeVec.z;
      double zita=tempp.z-nodePos.z;

      double tempt=(sqrt(pow(alpha,2)+pow(beta,2)*pow(R,2)-pow(alpha*sita-gama*zita-rosa*beta,2))-(alpha*beta+gama*sita+rosa*zita))/(pow(alpha,2)+pow(gama,2)+pow(rosa,2));
...
double 
PositionTable::OldGetAngle (Vector centrePos, Vector refPos, Vector node){
  double const PI = 4*atan(1);

  std::complex<double> A = std::complex<double>(centrePos.x,centrePos.y,centrePos.z);
  std::complex<double> B = std::complex<double>(node.x,node.y,node.z);
  std::complex<double> C = std::complex<double>(refPos.x,refPos.y,refPos.z);   //Change B with C if you want angles clockwise
