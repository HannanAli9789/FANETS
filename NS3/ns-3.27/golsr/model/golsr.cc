...
NS_LOG_COMPONENT_DEFINE ("golsrRoutingProtocol");
...
struct DeferredRouteOutputTag : public Tag
{
  ...
  {
    static TypeId tid = TypeId ("ns3::golsr::DeferredRouteOutputTag").SetParent<Tag> ();
    return tid;
  }
...
{
  static TypeId tid = TypeId ("ns3::golsr::RoutingProtocol")
...
NS_LOG_FUNCTION (this << p->GetUid () << header.GetDestination () << idev->GetAddress ());
  if (m_socketAddresses.empty ())
    {
      NS_LOG_LOGIC ("No golsr interfaces");
      return false;
    }
  NS_ASSERT (m_ipv4 != 0);
  NS_ASSERT (p != 0);
  ...
  if (m_ipv4->IsDestinationAddress (dst, iif))
    {
      ...
      if (!tHeader.IsValid ())
        {
          NS_LOG_DEBUG ("golsr message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
          return false;
        }
