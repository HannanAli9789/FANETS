                              ...
                              Ptr<NetDevice> oif, Socket::SocketErrno &sockerr)
{
        ...
        if (nextHop != Ipv4Address::GetZero ())
        {
                ...
                TypeHeader tHeader (GPSRTYPE_POS);
                PositionHeader posHeader (dstPos.x, dstPos.y, dstPos.z, updated, (uint64_t) 0, (uint64_t) 0, (uint64_t) 0, (uint8_t) 0, myPos.x, myPos.y, myPos.z);
...
RoutingProtocol::RecvGPSR (Ptr<Socket> socket)
{
        ...
        NS_LOG_DEBUG("update position"<<Position.x<<Position.y<<Position.z  );
        //更新neighbor的信息
